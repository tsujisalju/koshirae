...
function isCoinA(candidate: string, other: string): boolean {
  for (let i = 0; i < Math.max(candidate.length, other.length); i++) {
    const a = candidate.charCodeAt(i) ?? -1,
      b = other.charCodeAt(i) ?? -1;
    if (a !== b) return a > b;
  }
  return false;
}
...
async function main() {
  ...
  const [position, leftoverA, leftoverB] = tx.moveCall({
    target: `${CETUS_PACKAGE_ID}::poll_creator::create_pool_v3`,
